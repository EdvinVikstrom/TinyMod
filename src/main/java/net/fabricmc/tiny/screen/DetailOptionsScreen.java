...
public class DetailOptionsScreen extends Screen {
    ...
    public DetailOptionsScreen(Screen parent)
    {
        super(new TranslatableText("options.details"));
        this.parent = parent;
    }
    ...
    static {
        ...
        optionList.add(Option.CLOUDS);
        optionList.add(Option.PARTICLES);
        optionList.add(Option.ENTITY_SHADOWS);
        Set<String> keys = Config.getProperties().keySet();
        for (String key : keys)
        {
            AbstractProperty<?> property = Config.getProperty(key);
            if (property.getCategory() != Category.DETAILS)
                continue;
