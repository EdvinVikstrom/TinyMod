...
public class PerformanceOptionsScreen extends Screen {
    ...
    public PerformanceOptionsScreen(Screen parent)
    {
        super(new TranslatableText("options.performance"));
        this.parent = parent;
    }
    ...
    static {
        ...
        optionList.add(Option.MIPMAP_LEVELS);
        optionList.add(Option.ENTITY_DISTANCE_SCALING);
        Set<String> keys = Config.getProperties().keySet();
        for (String key : keys)
        {
            AbstractProperty<?> property = Config.getProperty(key);
            if (property.getCategory() != Category.PERFORMANCE)
                continue;
